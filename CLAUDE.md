...
- **M0 done; M1 in progress (215 deterministic tests).** M0: `core/` is a true leaf (graph wiring in `aerosense/graph.py`), routers in `core/routing.py`, import-invariant enforced recursively. M1: `core/eval/` golden safety-check harness + frozen baseline; `core/audit/` hash-chained DO-178C trace log; `core/prompts.py` versioned registry + sha256 lockfile for all 11 Gemini-calling phases; **`core/tracing/`** — sync, OTel-shaped span tracer (perf_counter-based; Windows `monotonic()` has ~15.6ms granularity, wrong tool for span duration). `call_gemini` gets an "llm" span (instruments all 11 callers via one shared helper); `aerosense/graph.py` wraps every node in a "node" span via `traced_node`; all 4 routers in `core/routing.py` emit "decision" spans. `tests/conftest.py` sets a dummy `GOOGLE_API_KEY` for CI. Remaining M1: HITL gate on Phase 10 (ground_stop). See `docs/superpowers/specs/2026-06-26-aerocommand-scope.md`.
