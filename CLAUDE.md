...
- **M0 + M1 done on `master` (253 deterministic tests).** M0: `core/` is a true leaf (graph wiring in `aerosense/graph.py`), routers in `core/routing.py`, import-invariant enforced recursively. M1: `core/eval/` golden safety-check harness + frozen baseline; `core/audit/` hash-chained DO-178C trace log; `core/prompts.py` versioned registry + sha256 lockfile for all 11 Gemini-calling phases; `core/tracing/` sync OTel-shaped span tracer (perf_counter-based — Windows `monotonic()` has ~15.6ms granularity, wrong tool for span duration) wired into `call_gemini`, every graph node, and all routers; **`core/hitl/`** — real human-approval gate: an active `ground_stop` (Phase 10's most disruptive action) pauses the compiled graph (`MemorySaver` + `interrupt_before=["hitl_gate"]`) via a new `route_after_tfm` router, resumes on `ApprovalGate.approve()`. `api/websocket/__init__.py` updated for the now-required `thread_id` config and to distinguish a real pause from completion.
- **⚠ Parallel-branch situation (2026-06-26):** a second autonomous session built **M2 → M3 → M4** (`cdm/` seam, AOC responder, distributed adapters, Docker) on branches `m2-cdm-seam`/`m3-aoc-responder`/`m3-e2e-seam`/`m4-distributed-adapters`, pushed to `origin` but **not merged to `master`**. It also has its own, different implementation of prompt-lockfile + tracing ("hardening"/"observability" commits `3c75a81`/`d45313e`) that likely overlaps with this session's `core/prompts.py`/`core/tracing/`. `origin/master` is NOT diverged (still at the pre-M0 commit), so this is safe to push, but **the two M1-equivalent implementations need manual reconciliation before merging the other branches** — don't blind-merge. See `Notes.md` (gitignored) for the other session's own account.
