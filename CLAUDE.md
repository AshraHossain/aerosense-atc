...
- **M0 done; M1 in progress (177 deterministic tests).** M0: `core/` is a true leaf (graph wiring in `aerosense/graph.py`), routers in `core/routing.py`, import-invariant enforced recursively. M1: `core/eval/` golden safety-check harness + frozen baseline; `core/audit/` hash-chained DO-178C trace log; **`core/prompts.py`** — versioned registry + sha256 lockfile for all 11 phase system prompts (phase_11 has none — pure compliance logic, no Gemini call) — every `phase_NN_*.py` now does `SYSTEM = get_prompt("phase_NN.system").template` instead of an inline string. `tests/conftest.py` sets a dummy `GOOGLE_API_KEY` for CI. Remaining M1: real Tracer wired into the graph, HITL gate on Phase 10 (ground_stop). See `docs/superpowers/specs/2026-06-26-aerocommand-scope.md`.
