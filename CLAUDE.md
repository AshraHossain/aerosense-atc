...
- **M0 done + M1 eval harness landed (120 deterministic tests).** M0: `core/` is a true leaf (graph wiring moved to `aerosense/graph.py`), routers extracted to `core/routing.py`, import-invariant enforced recursively. M1 (in progress): `core/eval/` — golden safety-check harness with a frozen pass-rate baseline (`python -m core.eval.run`), reusing AutoRedTeam's metrics; `tests/integration/` cross-validates against the real `simulation/` scenarios. `tests/conftest.py` sets a dummy `GOOGLE_API_KEY` for CI. Remaining M1: port hash-chain audit + prompt lockfile into `core/`, real tracer, HITL gate. See `docs/superpowers/specs/2026-06-26-aerocommand-scope.md`.
